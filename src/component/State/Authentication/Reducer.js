...
export const authReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case GET_USER_SUCCESS:
            return {
                ...state, isLoading: false, user: action.payload, favorites: action.payload.favorite
            }
        case ADD_TO_FAVORITE_SUCCESS:
            return {
                ...state, isLoading: false, error: null, favorites: isPresntInFavorites(state.favorites, action.payload) ? state.favorites.filter(item => item.id !== action.payload.id) : [action.payload, ...state.favorites]
            }
